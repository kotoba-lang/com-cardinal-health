(ns cardinal-health.main
  "Kotodama WASM entrypoint for the Cardinal_health clean-room actor (L4) — Clojure port.
  ...
  facts (`cardinal_health.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cardinal_health")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "cardinal_pre"
    :fields [:memberId :drugId :quantity :refills :status] :required [:quantity :refills]
    :coerce {:quantity :int :refills :int} :refs {}}
   {:entity "Claim" :plural "claims" :id-prefix "cardinal_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "cardinal_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "cardinal_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "cardinal_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {:memberId "Member"}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "cardinal_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"cardinal_health.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cardinal_health-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cardinal_health-compat/ready)
